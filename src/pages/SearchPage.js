  ...
  const SearchPage = () => {
    ...
    useEffect(() => {
      const getCategories = async () => {
        try {
          const { data } = await axios.get('/api/v1/products/find/categories');
          setCategories(data);
        } catch (err) {
      ...
      const getData = async () => {
        try {
          ...
          const { data } = await axios.get(
            `/api/v1/products/find/search?page=${page}&query=${query}&category=${category}&price=${price}&rating=${rating}&order=${order}`
          );
